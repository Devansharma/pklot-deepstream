...
namespace ParkingLot {
  ...
  int
  Pklot::create_input_sources(gpointer pipe, gpointer mux, guint num_sources) {
  ...
  void
  Pklot::changeBBoxColor(gpointer obj_meta_data, int has_bg_color, float red, float green,
  ...
  void
  Pklot::addDisplayMeta(gpointer batch_meta_data, gpointer frame_meta_data) {
  ...
  GstPadProbeReturn
  Pklot::tiler_src_pad_buffer_probe(GstPad *pad, GstPadProbeInfo *info,
                            gpointer u_data) {
  ...
  gboolean
  Pklot::bus_call(GstBus *bus, GstMessage *msg, gpointer data) {
  ...
  void
  Pklot::cb_newpad(GstElement *decodebin, GstPad *decoder_src_pad, gpointer data) {
  ...
  void
  Pklot::decodebin_child_added(GstChildProxy *child_proxy, GObject *object, gchar *name, gpointer user_data) {
  ...
  GstElement *
  Pklot::create_source_bin(guint index, gchar *uri) {
  ...
  gchar *
  Pklot::get_absolute_file_path(gchar *cfg_file_path, gchar *file_path) {
  ...
  gboolean
  Pklot::set_tracker_properties(GstElement *nvtracker) {
    gboolean ret = FALSE;
    GError *error = NULL;
    gchar **keys = NULL;
    gchar **key = NULL;
  ...
  int
  Pklot::configure_element_properties(int num_sources, GstElement *streammux, GstElement *pgie_yolo_detector,
    ...
    if (boost::filesystem::exists(boost::filesystem::path(
        ParkingLot::Pklot::PGIE_YOLO_ENGINE_PATH))) {

      g_object_set(G_OBJECT(pgie_yolo_detector),
                  "model-engine-file", ParkingLot::Pklot::PGIE_YOLO_ENGINE_PATH.c_str(), NULL);
    }
    else {
    ...
    if (!ParkingLot::Pklot::set_tracker_properties(nvtracker)) {
  ...
  void Pklot::setPaths(guint num_sources) {

    // Config Paths
    PGIE_YOLO_DETECTOR_CONFIG_FILE_PATH =
    strdup("models/pklot_Yolov5/config_infer_primary_yolov5s.txt");
...
int main(int argc, char *argv[]) {
  ...
  ctx = g_option_context_new("Pklot DeepStream App");
  group = g_option_group_new("space-empty", "space-occupied", NULL, NULL, NULL);
  g_option_group_add_entries(group, Pklot.entries);
  ...
  gint sources = Pklot.create_input_sources(pipeline, streammux, num_sources);
  if (sources == -1) {
    return -1;
  }
  else {
  ...
  int fail_safe = Pklot.configure_element_properties(num_sources, streammux, pgie_yolo_detector,
  ...
  bus_watch_id = gst_bus_add_watch(bus, Pklot.bus_call, loop);
  ...
  if (!tiler_src_pad) {
    ...
    gst_pad_add_probe(tiler_src_pad, GST_PAD_PROBE_TYPE_BUFFER,
                      Pklot.tiler_src_pad_buffer_probe, NULL, NULL);
  }
